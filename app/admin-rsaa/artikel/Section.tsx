...
const Section = () => {
    ...
    const [deleteSelect, setDeleteSelect] = useState<articleResponse | null>(null)
    ...
    const handleDelete = async (articleID: string) => {
        try {
            const response = await axios.delete(`${API_URL}/api/article/${articleID}`, {
                headers: {
                    ...
                    <div className="bg-white p-4 rounded-lg">
                        <div className="font-bold uppercase text-red-500">
                            <div className="">
                                ...
                                <div className="">
                                    <div className="text-xs p-2 bg-red-500 text-white w-fit rounded">judul artikel : {deleteSelect.title}</div>
                                </div>
                            </div>
                            <div className="flex gap-2 mt-5 justify-center">
                                <button onClick={() => handleDelete(`${deleteSelect.articleID}`)} className="btn btn-success text-white w-20">Ya</button>
                                ...
                                <React.Fragment key={index}>
                                    <div className="p-4 rounded-lg shadow-slate-100 shadow">
                                        ...
                                        <div className="flex justify-end gap-3 mt-4">
                                            <button onClick={() => navigation.push(`/admin-rsaa/artikel/edit/${item.articleID}`)} className='btn btn-warning'>
