...
const Section: React.FC<props> = ({ articleID }) => {
    ...
    const getData = async () => {
        try {
            ...
            if (response.data.success == true) {
                ...
                setValue('title', dataArticle.title ? dataArticle.title : '')
                setValue('desc', dataArticle.desc ? dataArticle.desc : '')
                setImagePreview(dataArticle.images ? dataArticle.images : '')
            }
        } catch (error) {
    ...
    const onSubmit: SubmitHandler<Inputs> = async (data) => {
        try {
            ...
            if (response.data.data) {
                ...
                setSuccess(true)
                setTimeout(() => {
                    navigation.push('/admin-rsaa/artikel')
                }, 3000)
            }
        } catch (error) {
            console.log(error);
    ...
    return (
        <div className=' p-5'>
            {success && <AlertSuccess title='edit artikel' />}
            <div className="rounded-xl shadow-md lg:w-[50%]">
                <div className="card-body">
                    <div className="uppercase font-bold">Form Edit artikel</div>
                    <div className="">
