...
const Section = () => {
    ...
    const onSubmit: SubmitHandler<Inputs> = async (data) => {
        try {
            const response = await axios.post(`${API_URL}/api/magang/post`, data, {
                headers: {
    ...
    return (
        <div className='p-5 flex justify-center'>
            {success && <AlertSuccess title='Add magang' />}
            <div className="rounded-xl shadow-md w-full lg:w-[70%]">
                <div className="card-body grid lg:grid-cols-4">
                    <label className="form-control w-full lg:col-span-2">
                        <div className="label">
                            <span className="label-text">Nama magang</span>
                        </div>
                        <input type="text"  {...register("name", { required: true })} placeholder="masukan nama magang" className="input input-bordered w-full" />
                        {errors.name &&
                            <div className="flex justify-end">
                                <span className='text-xs mt-2 p-1 bg-red-600 text-white rounded-sm'>* Nama magang Harus Di Isi</span>
                            </div>
                        }
                    </label>
                    <label className="form-control w-full">
                        ...
                        <input type="date" {...register("dateStart", { required: true })} placeholder="masukan nama magang" className="input input-bordered w-full" />
                        {errors.dateStart &&
                            <div className="flex justify-end">
                    ...
                    <label className="form-control w-full">
                        ...
                        <input type="date"  {...register("dateEnd", { required: true })} placeholder="masukan nama magang" className="input input-bordered w-full" />
                        {errors.dateEnd &&
                            <div className="flex justify-end">
