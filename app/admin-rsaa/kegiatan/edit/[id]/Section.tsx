...
const Section: React.FC<props> = ({ activityID }) => {
    ...
    const getData = async () => {
        try {
            const response = await axios.get(`${API_URL}/api/activity?activityID=${activityID}`)

            if (response.data.success == true) {
                const dataActivity: activityResponse = response.data.data
                setValue('title', dataActivity.title ? dataActivity.title : '')
                setValue('desc', dataActivity.desc ? dataActivity.desc : '')
                setImagePreview(dataActivity.images ? dataActivity.images : '')
            }
        } catch (error) {
    ...
    const onSubmit: SubmitHandler<Inputs> = async (data) => {
        try {
            ...
            const response = await axios.put(`${API_URL}/api/activity/${activityID}`, {
                title: data.title,
                desc: data.desc,
    ...
    return (
        <div className=' p-5'>
            {success && <AlertSuccess title='Edit Kegiatan' />}
            <div className="rounded-xl shadow-md lg:w-[50%]">
                <div className="card-body">
                    <div className="uppercase font-bold">Form Edit Kegiatan</div>
                    <div className="">
